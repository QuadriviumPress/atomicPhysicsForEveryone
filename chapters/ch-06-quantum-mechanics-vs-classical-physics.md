...
- A state for $\alpha$ can be written as a superposition of states for $\beta$, and vise-versa. For example,
...
- The square of the amplitudes in the above equations tell us the probability, upon measurement, we will find the system in that state with the value associated with that state. For example, if the system was in state $\psi _{\beta ,137}$ with value $\beta _{137}$ and we measured $\alpha$, the probability the system is now in a particular state of $\alpha$, say state $\psi _{\alpha ,3}$ with value $\alpha _3$, is the amplitude squared, $A_{\alpha ,3}^2$.
